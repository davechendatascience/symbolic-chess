...
def run_distillation(X_train, y_train, X_test, y_test, feature_names):
    ...
    # (predict_with_tree evaluates the tessera tree on the test matrix)
    eq_with_test = []
    for r in eqs:
...
def main():
    ...
    if not args.skip_pysr:
        print("\n--- PySR ---")
        eq_results = run_distillation(X_train, y_train, X_test, y_test, feature_names) or []
    else:
